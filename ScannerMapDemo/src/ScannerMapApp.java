...
//		System.out.println("You entered: "+inputStr);
		...
		while(sc1.hasNext()) {// while there is still input to consume
			System.out.println(sc1.next());
		}
		sc1.close();
		
		Map<String, String> statesMap = new HashMap<>();
		String statesString = "OH Ohio CA California IN Indiana CO Colorado";
		Scanner sc2 = new Scanner(statesString);
		// populate statesMap with data from statesString
		while (sc2.hasNext()) {
			String abbrev=sc2.next();
			String stateName=sc2.next();
			statesMap.put(abbrev, stateName);
		}
